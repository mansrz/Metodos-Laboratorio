...
class ProductLinkSerializer(ProductSerializer):
    ...
    """
    price_t = serializers.SerializerMethodField()
    currency = serializers.SerializerMethodField()

    class Meta(PublicProductSerializer.Meta):
        fields = overridable(
            "OSCARAPI_PRODUCT_FIELDS", default=("url", "id", "upc", "title", "price", "images",
                                                "price_t", "currency",)
        )

    def get_price_t(self, obj):
        from oscar.apps.partner.strategy import Selector
        price = Selector().strategy().fetch_for_product(obj)
        return price.price.excl_tax
